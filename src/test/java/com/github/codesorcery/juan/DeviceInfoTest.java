...
public class DeviceInfoTest {
    ...
    public void samsungGalaxyS8Active() {
        ...
        final String deviceId = getDeviceId(input);
        assertEquals("SM-G892A", deviceId);
        final Optional<DeviceInfo> deviceInfo = deviceLookup.getDeviceInfo(deviceId);
    ...
    public void samsungGalaxyS7edge() {
        ...
        final String deviceId = getDeviceId(input);
        assertEquals("SM-G935F", deviceId);
        final Optional<DeviceInfo> deviceInfo = deviceLookup.getDeviceInfo(deviceId);
        assertTrue(deviceInfo.isPresent());
    ...
    public void lenovoK3Note() {
        ...
        final String deviceId = getDeviceId(input);
        assertEquals("Lenovo K50a40", deviceId);
        final Optional<DeviceInfo> deviceInfo = deviceLookup.getDeviceInfo(deviceId);
        assertTrue(deviceInfo.isPresent());
    ...
    public void appleiPhone() {
        ...
        final String deviceId = getDeviceId(input);
        assertEquals("iPhone", deviceId);
        final Optional<DeviceInfo> deviceInfo = deviceLookup.getDeviceInfo(deviceId);
        assertTrue(deviceInfo.isPresent());
        Validators.validateDeviceInfo(deviceInfo.get(), "Apple", "iPhone");
    }

    private String getDeviceId(String uaString) {
        final TokenizedUserAgent ua = TokenizedUserAgent.forUserAgentString(uaString);
        final OperatingSystem os = OperatingSystem.fromUserAgent(ua);
        final String deviceId = os.getDeviceId();
